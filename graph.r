...
data_seq<- data.frame(read.csv2(file="var_size.csv",sep=';', dec='.'));
data_thd<- data.frame(read.csv2(file="thd_size.csv",sep=';', dec='.'));

stat_seq<-ddply(data_seq,c("size"),summarise,N=length(temps),mean=mean(temps))
stat_thd<-ddply(data_thd,c("size"),summarise,N=length(temps),mean=mean(temps))
p <- ggplot();

p <- p + geom_point(data=stat_seq, aes(x=size, y=mean, colour = "var_size")) + geom_line(data=stat_seq, aes(x=size, y=mean, colour = "seqs"));
p <- p + geom_point(data=stat_thd, aes(x=size, y=mean, colour = "thd_num")) + geom_line(data=stat_thd, aes(x=size, y=mean, colour = "thds"));
